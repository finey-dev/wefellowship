...
// How much the scroll's drag gesture can overshoot the RelightFluidRefreshing's
// displacement; max displacement = _kDragSizeFactorLimit * displacement.
const double _kDragSizeFactorLimit = 1.5;

// When the scroll ends, the duration of the progress indicator's animation
// to the RelightFluidRefreshing's displacement.
...
/// [RelightFluidRefreshing] far enough to demonstrate that they want the app to
/// refresh. The returned [Future] must complete when the refresh operation is
/// finished.
///
/// Used by [RelightFluidRefreshing.onRefresh].
typedef RelightRefreshCallback = Future<void> Function();

// The state machine moves through these modes only when the scrollable
// identified by scrollableKey has been scrolled to its min or max limit.
enum _FluidRefreshingMode {
...
class RelightFluidRefreshing extends StatefulWidget {
  const RelightFluidRefreshing({
    Key? key,
  ...
  final RelightRefreshCallback onRefresh;
  ...
  @override
  RelightFluidRefreshingState createState() => RelightFluidRefreshingState();
}

class RelightFluidRefreshingState extends State<RelightFluidRefreshing>
    with TickerProviderStateMixin<RelightFluidRefreshing> {
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    if (notification is ScrollStartNotification &&
      ...
      setState(() {
        _mode = _FluidRefreshingMode.drag;
      });
      return false;
    }
    ...
    if (indicatorAtTopNow != _isIndicatorAtTop) {
      if (_mode == _FluidRefreshingMode.drag ||
          _mode == _FluidRefreshingMode.armed)
        _dismiss(_FluidRefreshingMode.canceled);
      ...
      if (_mode == _FluidRefreshingMode.armed &&
      ...
      switch (_mode) {
        case _FluidRefreshingMode.armed:
          _show();
          break;
        case _FluidRefreshingMode.drag:
          _dismiss(_FluidRefreshingMode.canceled);
          break;
        default:
          // do nothing
  ...
  Future<void> _dismiss(_FluidRefreshingMode newMode) async {
    await Future<void>.value();
    // This can only be called from _show() when refreshing and
    // _handleScrollNotification in response to a ScrollEndNotification or
    // direction change.
    assert(newMode == _FluidRefreshingMode.canceled ||
        newMode == _FluidRefreshingMode.done);
    setState(() {
      _mode = newMode;
    });
    switch (_mode) {
      case _FluidRefreshingMode.done:
      ...
      case _FluidRefreshingMode.canceled:
        await _positionController.animateTo(0.0,
  ...
  void _checkDragOffset(double containerExtent) {
    assert(_mode == _FluidRefreshingMode.drag ||
        _mode == _FluidRefreshingMode.armed);
    double newValue =
  ...
  void _show() {
    assert(_mode != _FluidRefreshingMode.refresh);
    assert(_mode != _FluidRefreshingMode.snap);
    final Completer<void> completer = Completer<void>();
    _pendingRefreshFuture = completer.future;
    _mode = _FluidRefreshingMode.snap;
    ...
    _springController
      ...
      if (mounted && _mode == _FluidRefreshingMode.snap) {
        setState(() {
          // Show the indeterminate progress indicator.
          _mode = _FluidRefreshingMode.refresh;
        });
        ...
        refreshResult.whenComplete(() {
          if (mounted && _mode == _FluidRefreshingMode.refresh) {
            completer.complete();

            _dismiss(_FluidRefreshingMode.done);
          }
        });
      }
    });
  ...
  ///
  /// Creating the [RelightFluidRefreshing] with a [GlobalKey<RelightFluidRefreshingState>]
  /// makes it possible to refer to the [RelightFluidRefreshingState].
  ///
  /// The future returned from this method completes when the
  /// [RelightFluidRefreshing.onRefresh] callback's future completes.
  ///
  ...
  Future<void>? show({bool atTop = true}) {
    if (_mode != _FluidRefreshingMode.refresh &&
        _mode != _FluidRefreshingMode.snap) {
                ...
                child: Transform(
                  ...
                  child: CircularProgress(
                    ...
                    innerCircleRadius: height *
                        15 /
                        100 * // 15.0
                        ((_mode != _FluidRefreshingMode.done)
